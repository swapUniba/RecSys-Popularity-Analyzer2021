...
OUTPUT_FILE_NAME = 'personalized_pagerank_property'  # output filename (NO NEED TO ADD .csv)
PROPERTY = True  # add properties
######################################################################################
...
#Creazione del grafo G
G = nx.DiGraph()

#Creazione set di utenti del nodo
G.add_nodes_from(user)

#Creazione di un insieme di nodi di elementi
G.add_nodes_from(item1)

#aggiunta di Edge tra i nodi (cioè bordi)
#i bordi rappresentano le relazioni tra le varie entità
i = 0
while i < len(user):
...
#funzione che ci permette di mostrare il grafo
def show_graph(Graph):
    nx.draw(G, with_labels=True, font_weight='bold')
    plt.show()

#Funzione di gestione degli articoli non valutati dall'utente
...
#funzione che gestisce gli articoli valutati dall'utente
def rated_item(user):
    cont = 0
    it1 = []
    for x in user1:
...
if PROPERTY:
    ...
    dt = pd.read_csv('../datasets/books-tags.csv')
    it = []
    it = dt['item']
    prop = []
    prop = dt['tags']
    ...
    # Creazione del set di proprietà dei nodi e aggiunta
...
for u in user_set:
    print(u)
    # Creazione del dizionario per la personalizzazione del pagerank per l'utente u
    customized = {}
    tot = 0
    supp = []
    list1 = G[u]
