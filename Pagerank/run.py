...
OUTPUT_FILE_NAME = 'pagerank_property'               # output filename (NO NEED TO ADD .csv)
...
# Creazione di un insieme di nodi utente
G.add_nodes_from(user)

# Creazione di un insieme di nodi di elementi
G.add_nodes_from(item1)

# Aggiunta di Edge tra i nodi(per ponderare i bordi)
i = 0
while i < len(user):
...
if PROPERTY:
    ...
    df = pd.read_csv('../datasets/books-tags.csv', encoding='latin-1')
    it = []
    it = df['item']
    prop = []
    prop = df['tags']
