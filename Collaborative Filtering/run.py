...
biased_mf_50 = als.BiasedMF(50)             #algoritmo che permette di tener conto dei pregiudizi dell'utente
biased_mf_100 = als.BiasedMF(100)
biased_mf_200 = als.BiasedMF(200)
implicit_mf_50 = als.ImplicitMF(50)            #riguarda tutte le valutazioni implcite, che il sistema deduce da azioni dell'utente(acquisti, like, ricerche ecc...)
...
bayesian = BPR()                                                #fattorizzazione di matrici che permette la previsione colalborativa
hierarchical_poisson_fact_50 = HPF(50)                          #modelli di fattorizzazione a matrice non negativa basati su una struttura gerarchica
hierarchical_poisson_fact_100 = HPF(100)
hierarchical_poisson_fact_200 = HPF(200)

train, test = train_test_split(data[['user', 'item', 'rating']], test_size=0.2)


eval = batch.MultiEval('../recs/cf', recommend=NUM_OF_RECS)
eval.add_datasets((train, test), name='ratings')
